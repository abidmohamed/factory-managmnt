...
def transaction_list(request):
    ...
    customerpayments = CustomerPayment.objects.all().filter(pay_status='Cash')
    supplierpayments = SupplierPayment.objects.all().filter(pay_status='Cash')
    total_per_period = 0
    income_per_period = 0
    ...
    if request.method == 'POST':
        ...
        customerpayments = CustomerPayment.objects.all().filter(
            Q(
        ...
        supplierpayments = SupplierPayment.objects.all().filter(
            Q(
                pay_date__gt=date(int(start_year), int(start_month), int(start_day)),
                pay_date__lt=date(int(end_year), int(end_month), int(end_day))
            )
